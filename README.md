...
Supported profile routes are `/in/<you>/` and `/in/<you>/recent-activity/shares/` (also `all/`, filtered to your own posts). The native post composer is allowed on the feed and your own profile/post routes. On an explicit **New post** request, the extension opens a recognized native **Start a post** control if LinkedIn ignores the URL shortcut, while preserving the invisible page layout needed by lazy-loaded controls. Initialization checks stop after 20 seconds, with at most ten native opening attempts. Closing the editor stops automatic opening. **New post** on the same page or **Open editor** explicitly retries; clicking New post while writing focuses the existing editor without discarding the draft. **Reload posting page** is available if native controls never appear. Composer recognition supports both older class names and newer dialog layouts identified by an editor and a Post button; native publishing footers remain visible. Recognized composer settings dialogs are allowed; unfamiliar media/audience dialogs may still need selector updates. Profile detail/editor overlays, standalone article readers, private sharing, and other routes remain masked. Inline comments are supported. The extension never clicks Post, submits a draft, or calls LinkedIn APIs for you. It does not auto-scroll; native scroll-triggered loading and recognized **Load more posts** buttons remain available on your posts page.
...
`extension/` is the complete distributable. `dist/linkedin-quiet-0.2.2.zip` contains the extension plus its license, README, and privacy notice. The packaging script requires the standard `zip` command.
...
If your environment cannot launch Playwright, run `node scripts/browser-check.mjs` and open `http://127.0.0.1:8765`. This executes 31 synthetic visibility/navigation/composer/loading checks using the production CSS and scripts, with test-only localhost URL and Chrome-storage adapters. Version 0.2.2 passed all 31 checks in the Codex in-app browser and three Node policy tests. The lazy-trigger and delayed-handler regressions were also run against version 0.2.1: both failed there and passed with this update. These reproduce two code-level failure modes; the screenshot alone cannot establish which caused the live issue. The 100-post fixture verified zero author rechecks after an unrelated mutation and exactly one after one author-link edit. This measures extension work, not end-to-end LinkedIn server latency. Composer tests submit only a local fixture; no real LinkedIn post was published. The full Playwright suite and actual unpacked-extension smoke test were not executed successfully in the authoring sandbox because it blocked Chromium launch.
