(() => {
  ...
  function render() {
    ...
    if (statusKey !== key) {
      ...
      shadow.querySelector('#heading').textContent = composeSession ? 'A space for your next idea.' : mode === 'blocked' ? 'Nothing to catch up on.' : 'Your space is quiet.';
      shadow.querySelector('#description').textContent = composeSession
        ? 'Use LinkedIn’s Start a post button if the editor hasn’t opened yet. When you’re done, My posts takes you back to your work. Your feed stays hidden.'
